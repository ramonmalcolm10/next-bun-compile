...
and next-bun-compile handles that without configuration.
...
2. **Walks nested `node_modules/`.** Monorepo standalone outputs often
   ...
   `standalone/apps/web/node_modules/`. next-bun-compile scans both for
...
next-bun-compile looks for `server.js` in the standalone tree but skips
