...
<CardGrid>
  ...
  <Card title="Faster cold start" icon="random">
    The server core is pre-bundled and minified into the binary — no
    module resolution or node_modules walks at boot. Real Next.js
    apps start ~2x faster than `next start` on the same runtime.
  </Card>
  ...
  <Card title="Works with what you already use" icon="approve-check">
    One line in your `next.config.ts` (`output: "standalone"`). No
